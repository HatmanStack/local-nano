...
export default class TransformersBackend extends PolyfillBackend {
  ...
  async #ensureGenerator(monitorTarget) {
    if (!this.#generator) {
      ...
      // Aggregate per-file `progress` events into an overall fraction.
      // Transformers.js never emits `progress_total`, only per-file `progress`.
      ...
      const progress_callback = (data) => {
        if (data.status === 'progress' && data.file) {
          ...
          if (total > 0) {
            fileBytes.set(data.file, { loaded, total });
            let l = 0, t = 0;
            for (const v of fileBytes.values()) { l += v.loaded; t += v.total; }
            if (t > 0) dispatch(Math.min(l / t, 0.999));
          }
        } else if (data.status === 'done' && data.file) {
          const cur = fileBytes.get(data.file);
          if (cur) cur.loaded = cur.total;
