...
declare global {
  interface Window {
    /** Set once this script has built its panel in this tab. */
    __localNanoLoaded?: true;
  }
}
...
// The service worker re-injects this file into tabs that predate the install
// or auto-update (see `injectAndRetry` in src/background/handler.ts). That can
// land in a tab where the declared content script is already present, or race
// a still-loading one, so a second run must not build a second panel.
if (!window.__localNanoLoaded) {
  window.__localNanoLoaded = true;
  initContentScript();
}
