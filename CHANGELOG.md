...
- **Model-load progress and failure surfacing.** While the model loads, a system bubble shows a live elapsed counter (proof of life for the multi-GB first-run download) and, if it drags past ~45s, appends remedies without giving up — the load is never killed on a timer, so a slow-but-healthy download isn't false-failed. A genuine load error (the offscreen session reports failure) shows the error and a Retry button.
