...
describe('initSession — streaming', () => {
  ...
  it('fires touchIdle once on a successful panel-open load (idle window arms at load, not only on generation)', async () => {
    const deps = makeDeps();
    initSession(deps);
    const listener = getToggleListener();
    listener(TOGGLE_MESSAGE); // open -> warmup -> successful load
    await flushMicrotasks();
    expect(warmupSessionMock).toHaveBeenCalledTimes(1);
    // The load arms the idle-release window so a model loaded but never used is
    // still reclaimed after the timeout (it does not wait for a first generation).
    expect(touchIdleMock).toHaveBeenCalledTimes(1);
  });
